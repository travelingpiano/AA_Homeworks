...
def below_average_years
  ...
  bad_movies = Movie.where('score < ?',5)
  bad_movies.select('yr','COUNT(*) as bad_movies').group('yr').order('bad_movies DESC')
end
...
def uma_movies
  ...
  Movie.select('movies.id','movies.title','movies.yr').joins(:actors).where('actors.name = ?','Uma Thurman').order('movies.yr')
end
